...
const Pagination = ({ queries }) => {
    ...
    const [page, setPage] = React.useState(pagination[0].startIndex)
    ...
    return (
        <div className="flex flex-col items-center py-14 max-w-[700px]">
            <div className="relative text-[#4285f4]">
                ...
                <img className='w-[260px] md:w-[300px]' src={Logo} alt="" />
                {
                    queries.nextPage && (
            ...
            <div className="flex gap-3 text-[#4285f4] text-sm">
                {
                    pagination.map((index) => (
                        <span key={index.page} onClick={() => paginationClickHandler(index.startIndex)}
                            className={`cursor-pointer ${page === index.startIndex} ? 'text-[#000]' : ''`}>
                            {index.page}
                        </span>
                    ))
                }
